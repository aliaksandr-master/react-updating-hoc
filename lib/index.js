...
export const AFTER_INIT_AND_UPDATE = `@AFTER_INIT_UPDATE@${base}@`;


const methodsMap = {
  [INIT]: [ 'componentDidMount' ],
  [BEFORE_UPDATE]: [ 'componentWillUpdate' ],
  [AFTER_UPDATE]: [ 'componentDidUpdate' ],
  [AFTER_INIT_AND_UPDATE]: [ 'componentDidMount', 'componentDidUpdate' ],
};
...
const methodsToFunction = {
  ...
  componentWillUpdate: (funcs) => function (nextProps) {
...
export const callWhen = (condition, executor, when = AFTER_INIT_AND_UPDATE) => {
...
export const callWhenPropsAreUpdated = (propNames, executor, useGet = false) => {
  ...
  return callWhen(
    (props, prevProps) => prevProps === null ? false : propNames.some((propName) => getProp(prevProps, propName) !== getProp(props, propName)),
    executor,
    AFTER_UPDATE
  );
};
...
export const callWhenPropsAreUpdatedDeeply = (propNames, executor, useGet = false) => {
  if (!Array.isArray(propNames)) {
    throw new TypeError(`propNames must be array of strings`);
  }
  ...
  return callWhen(
    (props, prevProps) => prevProps === null ? false : propNames.some((propName) => !isEqual(getProp(prevProps, propName), getProp(props, propName))),
    executor,
    AFTER_UPDATE
  );
};
...
export default (...triggers) => (Target) => {
  triggers = triggers
    .map((trigger) => {
      if (typeof trigger === 'function') {
        return callWhenAreInitialized(trigger);
      }
  ...
  const methodsNames = Obejct.keys(triggersByMethods);
